...
usuário quer pular — a skill `analizza-integration-test`, passando como
padrões o que este scaffold já decidiu (ela confirma cada valor antes de
seguir, conforme o próprio contrato dela — os padrões só evitam repetir
perguntas cuja resposta já se sabe):

```
escopo=ambos linguagem={language} banco=postgres layout=dedicado
```

- `escopo=ambos` — o scaffold sempre cria `{project-name}-web` (Next.js) e
  `{project-name}-mobile` (Expo) além do backend, então os guardrails de
  frontend (typecheck, Vitest, jest-expo) fazem sentido junto com os de
  backend.
- `linguagem={language}` — a mesma escolhida no Passo 2.
- `banco=postgres` — o único banco que este scaffold sobe, via
  `docker-compose.yml`.
- `layout=dedicado` — o projeto já nasce multi-módulo
  (`{project-name}-api` + `{project-name}-core` + `buildingBlocks`), então o
  módulo dedicado `{base}-integration-tests` é o layout coerente, não o
  módulo existente.

A `analizza-integration-test` constrói a infraestrutura descrita (e ainda não
implementada) na seção "Testes" das convenções gravadas no Passo 8:
`BaseIntegrationTest`, Testcontainers, as tasks `test`/`integrationTest`, a
regra ArchUnit que exige IT para todo entrypoint, dublês de saída e cobertura
JaCoCo — e, no frontend, typecheck, Vitest e jest-expo.

O relatório dela fica separado do relatório do Passo 11 — não repita nem
resuma o checklist de conclusão dela aqui; cada skill responde pelo que fez.
...
`analizza-integration-test`, que constrói essa infraestrutura como última
tarefa do scaffold.
